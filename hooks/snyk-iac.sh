...
if find . \( -name '*.tf' -o -name '*.hcl' -o -name '*.yml' -o -name '*.yaml' -o -name '*.json' -o -name '*.tfvars' -o -name '*.tfstate' -o -name '*.tfstate.backup' -o -name '*.tf.json' ! -name '.pre-commit-config.yaml' \) -print -quit; then
  snyk iac test "$@"
else
